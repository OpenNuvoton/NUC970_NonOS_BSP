			...
			if (element < USB_STOR_SCSI_SENSE_10_HDRSZ)
			{
				...
				for (j = 0; j < sg[i].length; j++)
				{
					if (element < USB_STOR_SCSI_SENSE_HDRSZ)
					{
						...
						the6->array[element] = (UINT8 *)&(sg[i].address[j]);
						the10->array[element] = (UINT8 *)&(sg[i].address[j]);
					}
...
static INT  usb_stor_scsiSense6to10(SCSI_CMD_T* the6)
{
	...
	UINT8   tempBuffer[USB_STOR_SCSI_SENSE_10_HDRSZ-USB_STOR_SCSI_SENSE_HDRSZ], *buffer=0;
	INT     outputBufferSize = 0;
	INT     length=0;
	INT     i = 0, j = 0, element = 0;
	INT     sb = 0, si = 0, db = 0, di = 0;
...
static void  fix_inquiry_data(SCSI_CMD_T *srb)
{
    ...
    if (srb->use_sg) 
    {
        ...
        sg = (SCATTER_LIST_T *) srb->request_buff;
        data_ptr = (UINT8 *) sg[0].address;
    } 
    else
        data_ptr = (UINT8 *)srb->request_buff;
...
void  UMAS_Qic157Command(SCSI_CMD_T *srb, UMAS_DATA_T *umas)
{
     ...
     *  a UINT8 cmnd[12], so we know we have storage available
     */
