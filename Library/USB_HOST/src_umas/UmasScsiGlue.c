...
FATFS_DRV_T * fatfs_drive_alloc()
{
	...
	sysprintf("No free FATFS USB drive slots!\n");
	return NULL;
}
...
static UINT8 usb_stor_sense_notready[18] =
{   
...
int  usbh_umas_read(int drv_no, unsigned int sec_no, int sec_cnt, unsigned char *buff)
{
	...
	UMAS_DATA_T 	*umas = umas_drive->umas;
	FATFS_DRV_T     *fdrv;
	SCSI_CMD_T  	*srb;
	UINT8 			*old_rq_buff;
	INT				retry = 10;
...
do_retry:
	...
	srb->request_buff = (UINT8 *)((UINT32)buff | NON_CACHE_MASK);
...
INT  UMAS_InitUmasDevice(UMAS_DATA_T *umas)   
{
	...
	for (lun = umas->max_lun; lun >= 0; lun--)
	{
		...
		if ((umas_drive = malloc(sizeof(UMAS_DRIVE_T))) == NULL)
			break;
		...
		if (fdrv == NULL)
		{
			sysprintf("No free FATFS drive slots!\n");
			continue;
		}
