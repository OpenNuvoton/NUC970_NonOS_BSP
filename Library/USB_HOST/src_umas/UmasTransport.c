 ...
 *  given SCSI command
 */
static UINT32  usb_stor_transfer_length(SCSI_CMD_T *srb)
{
	INT     i;
	INT     doDefault = 0;
	UINT32  len = 0;
	...
	else
	{
		switch (lengths[srb->cmnd[0]]) 
		{
			...
			case '9':
				...
				break;
			case 'G':
				len = (((UINT32)srb->cmnd[3])<<8) | srb->cmnd[4];
				break;
			case 'H':
				len = (((UINT32)srb->cmnd[7])<<8) | srb->cmnd[8];
				break;
			case 'I':
				len = (((UINT32)srb->cmnd[8])<<8) | srb->cmnd[9];
				break;
			case 'R':
				len = (((UINT32)srb->cmnd[2])<<16) | 
					  (((UINT32)srb->cmnd[3])<<8) | srb->cmnd[4];
				break;
			case 'S':
				len = (((UINT32)srb->cmnd[3])<<16) |
					   (((UINT32)srb->cmnd[4])<<8) | srb->cmnd[5];
				break;
			case 'T':
				len = (((UINT32)srb->cmnd[6])<<16) |
					   (((UINT32)srb->cmnd[7])<<8) | srb->cmnd[8];
				break;
			case 'U':
				len = (((UINT32)srb->cmnd[7])<<16) |
					   (((UINT32)srb->cmnd[8])<<8) | srb->cmnd[9];
				break;
			case 'C':
				len = (((UINT32)srb->cmnd[2])<<24) |
					   (((UINT32)srb->cmnd[3])<<16) |
					   (((UINT32)srb->cmnd[4])<<8) | srb->cmnd[5];
				break;
			case 'D':
				len = (((UINT32)srb->cmnd[6])<<24) |
					   (((UINT32)srb->cmnd[7])<<16) |
					   (((UINT32)srb->cmnd[8])<<8) | srb->cmnd[9];
				break;
			case 'V':
				len = 20;
 ...
 */
static INT  usb_stor_control_msg(UMAS_DATA_T *umas, UINT32 pipe,
								 UINT8 request, UINT8 requesttype, UINT16 value, 
								 UINT16 index, void *data, UINT16 size)
{
	...
	/* fill the URB */
	usb_fill_control_urb(urb, umas->pusb_dev, pipe, (UINT8 *)dr, data, size, 
							  ...
							  UINT32 len, UINT32 *act_len)
{
	...
	if (urb->status == -EINPROGRESS) 
	{
		...
		t0 = umas_get_ticks();
		while (umas_get_ticks() - t0 < 1000)
		{
			if (is_urb_completed()) 
				break;
		}

		if (umas_get_ticks() - t0 >= 1000)
		{
...
void  UMAS_InvokeTransport(SCSI_CMD_T *srb, UMAS_DATA_T *umas)
{
	...
	if (need_auto_sense) 
	{
		...
		UINT16  old_sg;
		UINT32  old_request_bufflen;
		UINT8   old_sc_data_direction;
		UINT8   old_cmnd[MAX_COMMAND_SIZE];
...
INT  UMAS_CbiTransport(SCSI_CMD_T *srb, UMAS_DATA_T *umas)
{
	...
	if (umas->subclass == UMAS_SC_UFI) 
	{
		...
		{
			if (((UINT8*)umas->irq_urb->transfer_buffer)[0])
				return USB_STOR_TRANSPORT_FAILED;
			else
				return USB_STOR_TRANSPORT_GOOD;
		}
...

{
	...
	while (sysGetTicks(TIMER0) - t0 < 10)
		;
}	
