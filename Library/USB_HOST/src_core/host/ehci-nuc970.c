...
static struct device  nuc900_ehci_dummy_dev;
...
static int usb_nuc900_probe(const struct hc_driver *driver)
{
    ...
    hcd = usb_create_hcd(driver, &nuc900_ehci_dummy_dev, "nuc900-ehci");
    if (!hcd) {
        retval = -ENOMEM;
        goto err1;
    }
    ...
    hcd->regs = (void *)USBH_BA;

    ehci->caps = (struct ehci_caps *)USBH_BA;			// hcd->regs;
    ehci->regs = (struct ehci_regs *)(USBH_BA+0x20);	// hcd->regs + HC_LENGTH(readl(&ehci->caps->hc_capbase));
...
void usb_nuc900_remove(struct usb_hcd *hcd)
{
        usb_remove_hcd(hcd);
        //iounmap(hcd->regs);
...
static int ehci_nuc900_start (struct usb_hcd *hcd)
{
    int ret;
...
static const struct hc_driver ehci_nuc900_hc_driver = {
	hcd_name,
    "Nvoton nuc970 EHCI Host Controller",
    sizeof(struct ehci_hcd),
    HCD_USB2|HCD_MEMORY,		// flags
    __ehci_reset,				// reset
    ehci_nuc900_start,			// start
    ehci_stop,					// stop
    NULL,						// shutdown
...
int usb_ehci_init(void)
{
	if (usb_disabled())
    	return -ENODEV;
	return usb_nuc900_probe(&ehci_nuc900_hc_driver);
}
