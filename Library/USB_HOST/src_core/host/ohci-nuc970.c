...
static struct device  nuc900_ohci_dummy_dev;
...
static int usb_nuc900_probe(const struct hc_driver *driver)
{
	...
	hcd = usb_create_hcd(driver, &nuc900_ohci_dummy_dev, "nuc900-ohci");
    if (!hcd)
        return -ENOMEM;
	...
	hcd->regs = (void *)(USBO_BA);	
    ohci->regs = (struct ohci_regs *)(USBO_BA);	
    
    retval = usb_add_hcd(hcd, 0, 0);
    if (retval == 0)
        return retval;

	USB_error("OHCI usb_nuc900_probe failed!\n");
  	usb_put_hcd(hcd);
  	return retval;
}
...
static const struct hc_driver ohci_nuc900_hc_driver = {
	hcd_name,
    "Nvoton nuc970 OHCI Host Controller",
    sizeof(struct ohci_hcd),
    HCD_USB11|HCD_MEMORY,		// flags
    NULL,						// reset
    ohci_nuc900_start,			// start
    ohci_stop,					// stop
    NULL,						// shutdown
...
int usb_ohci_init(void)
{
	if (usb_disabled())
    	return -ENODEV;
	return usb_nuc900_probe(&ohci_nuc900_hc_driver);
}
