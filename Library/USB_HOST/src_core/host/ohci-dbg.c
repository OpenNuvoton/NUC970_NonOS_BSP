...
{
	switch (ed_type) { 
	case PIPE_CONTROL:	return str_ctrl;
	case PIPE_BULK:		return str_bulk;
	case PIPE_INTERRUPT:	return str_intr;
	default:		return str_isoc;
	}
//	return NULL;
}	
#endif	
...
urb_print(struct urb * urb, char * str, int small, int status)
{
	...
	if (status != 0)
#endif
	USB_debug("%s %p dev=%d ep=%d%s-%s flags=%x len=%d/%d stat=%d",
		    str, urb, usb_pipedevice (pipe), usb_pipeendpoint (pipe),
		    usb_pipeout (pipe) ? "out" : "in",
		    //pipestring (pipe), urb->transfer_flags, urb->actual_length,
