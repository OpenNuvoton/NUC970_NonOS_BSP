...
{
	UINT32  flag;  /* 0:free, 1:allocated, 0x3:first block */
	UINT32  bcnt;  /* if allocated, the block count of allocated memory block */
	UINT32  magic;
	UINT32  reserved;
}  USB_MHDR_T;

__align(USB_MEM_BLOCK_SIZE) UINT8  _USBMemoryPool[USB_MEMORY_POOL_SIZE];

static USB_MHDR_T  *_pCurrent;
static UINT32  _FreeMemorySize;
UINT32  _AllocatedMemorySize;   
UINT32  *_USB_pCurrent = (UINT32 *)&_pCurrent; 

static UINT32  _MemoryPoolBase, _MemoryPoolEnd;  
...
{
	//sysSetCachePages((UINT32)_USBMemoryPool, USB_MEMORY_POOL_SIZE, CACHE_DISABLE);
	//sysFlushCache(D_CACHE);
	_MemoryPoolBase = (UINT32)&_USBMemoryPool[0] | NON_CACHE_MASK;
	//_MemoryPoolBase = (UINT32)&_USBMemoryPool[0];
...
INT32  USB_available_memory()
{
	return _FreeMemorySize;
}


INT32  USB_allocated_memory()
{
